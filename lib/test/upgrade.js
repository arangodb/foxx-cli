...
describe("Foxx service upgraded", () => {
  ...
  it("should succeed when mount is invalid but using --force", async () => {
    ...
    await foxx(
      `upgrade --force ${mount} ${path.resolve(
        basePath,
        "minimal-working-service.zip"
      )}`
    );
  ...
  it("should succeed when mount is invalid but using -f", async () => {
    await db.uninstallService(mount);
    try {
      await db.route(mount).get();
      expect.fail();
