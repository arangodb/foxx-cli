...
export const command = 'upgrade <mount-path> [source]'
export const description = 'Upgrade a mounted service'

const describe = description

const args = [
  ['mount-path', 'Database-relative path the service is mounted on'],
  ['source', `URL or file system path of the upgrade service. Use ${bold('-')} to pass a zip file from stdin`, '[default: "."]']
]
...
export function handler (argv) {
  ...
  .then((server) => {
    ...
    return upgrade(argv, server, opts)
  })
  .catch(fatal)
}
...
async function upgrade (argv, server, opts) {
  ...
  const result = await db.upgradeService(
    server.mount,
    source,
